...
The sequence after closing is: `blacksmith progress add` → run `blacksmith finish` → STOP.
...
## Task Selection
Pick ONE task from the ready queue. **Always pick the highest-priority (lowest number) ready task.** Only deviate if progress history explains why a specific lower-priority task should go next (e.g., it's a quick follow-up to the last session's work).
...
- **2+ prior failures**: Do NOT attempt implementation. Instead, decompose the bead into smaller sub-beads:
  ...
  3. Add a progress entry noting the decomposition, then exit cleanly via `blacksmith finish`
...
5. **Finish** — add a progress entry and call `blacksmith finish`, then STOP (Rule C):
   - **Add a progress entry** with a short handoff note:
     - What you completed this session
     - Current state of the codebase
     - Suggested next tasks for the next session
   - **Example command**:
     ```bash
     printf '## Session Handoff\n- Completed: ...\n- Current state: ...\n- Next: ...\n' | blacksmith progress add --bead-id bd-X --stdin
     ```
   - **Run the finish command**:
     ```bash
     blacksmith finish bd-X "<brief description>" src/file1.rs src/file2.rs
     ```
     This runs quality gates (check + test), verifies bead deliverables, then handles: staging, committing, bd close, bd sync, auto-committing .beads/, and git push — all in one command.
...
- **Turns 66-75**: **Emergency wrap-up.** If tests/lint are failing, make minimal fixes. If you can't fix in 10 turns, revert your changes (`git checkout -- .`), mark the failure (see below), add a progress entry, and exit cleanly.
- **Turn 76+**: **Hard stop.** Do NOT start any new work. If you haven't committed yet: revert, mark the failure, add a progress entry, and exit immediately. An uncommitted session is worse than a cleanly abandoned one.
...
- If task cannot be completed, mark the failure (see above), add a progress entry, exit cleanly
