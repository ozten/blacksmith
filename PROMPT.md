...
Bounded batching rule: Cap each assistant message at 2-4 tool calls (hard max 6), and count nested/inner calls inside wrapper tools (for example `multi_tool_use.parallel`) toward that same limit (so wrapper fanout cannot bypass the cap). Do NOT fan out dozens of speculative reads/greps/commands in one turn. Batch only calls you are certain you need right now, then inspect results before issuing the next batch.
